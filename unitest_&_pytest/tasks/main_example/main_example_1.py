...
def request_for_doc():
    ...
    return new_doc, new_doc_shelf_number


def add_new_doc(new_doc, new_doc_shelf_number):  # test_a
    documents.append(new_doc)
    append_doc_to_shelf(new_doc["number"], new_doc_shelf_number)
...
def secretary_program_start():
    ...
    while True:
        ...
        if user_command == 'p':
            ...
            new_doc, new_doc_shelf_number = request_for_doc()
            add_new_doc(new_doc, new_doc_shelf_number)
            print('\nНа полку "{}" добавлен новый документ:'.format(new_doc_shelf_number))
        elif user_command == 'd':
            doc_number, value = delete_doc()
            if value:
                print('Документ c номером "{}" был успешно удален'.format(doc_number['number']))
            else:
...
if __name__ == '__main__':
    ...
    # print(delete_doc() not in documents)
    print(documents)
    print(directories)
